...
RSpec.describe Hub::AssignedClientsController do
  describe "#index" do
    ...
    context "as an authenticated user" do
      ...
      let!(:assigned_to_me) { create :client, vita_partner: organization, intake: (build :intake), tax_returns: [(build :gyr_tax_return, :intake_ready, assigned_user: user)] }
      let!(:assigned_to_me_two_trs) { create :client, vita_partner: organization, intake: (build :intake), tax_returns: [(build :gyr_tax_return, :review_reviewing, assigned_user: user, year: 2019), (build :gyr_tax_return, :intake_ready_for_call, assigned_user: user)] }
      let!(:not_assigned_to_me) { create :client, vita_partner: organization, intake: (build :intake), tax_returns: [(build :gyr_tax_return, :intake_in_progress)] }
      ...
      context "filtering" do
        ...
        context "filtering and sorting" do
          let!(:starts_with_a_assigned) { create :client, vita_partner: organization, intake: (build :intake, preferred_name: "Aardvark Alan"), tax_returns: [(build :gyr_tax_return, :intake_in_progress, assigned_user: user)] }
      ...
      context "tax return count" do
        ...
        let!(:pagination_assigned_to_me) { 47.times { create :client, vita_partner: organization, intake: (build :intake), tax_returns: [(build :gyr_tax_return, :intake_ready, assigned_user: user)] } }
        let(:params) do
          {
            page: "1"
          }
