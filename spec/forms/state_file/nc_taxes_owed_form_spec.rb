...
RSpec.describe StateFile::NcTaxesOwedForm do
  ...
  context "when paying electronically" do
    let(:expected_deadline) { Date.new(2024, 4, 15) }

    before do
      allow(StateFile::StateInformationService).to receive(:payment_deadline_date).and_return(expected_deadline)
    end
