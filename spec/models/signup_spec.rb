...
RSpec.describe Signup, type: :model do
  ...
  describe ".valid_emails" do
    context "with signups that have valid emails and invalid emails" do
      before do
        ...
        create_list(:signup, 2, email_address: "beatrice@example.com", name: "Beatrice Basil")
        Signup.create!(email_address: "spinach@example.com", name: "Sally Spinach")
      end

      it "returns each valid email and the client's name" do
        expect(Signup.valid_emails).to match_array [['beatrice@example.com', 'Beatrice Basil'], ['spinach@example.com', 'Sally Spinach']]
      end
    end
  end
  ...
  describe ".send_followup_emails", active_job: true do
    ...
    before do
      allow(Signup).to receive(:valid_emails).and_return([['beatrice@example.com', 'Beatrice Basil'], ['spinach@example.com', 'Sally Spinach']])
    end
