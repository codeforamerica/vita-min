...
describe Efile::DependentEligibility::EipThree do
  context "when puerto_rico_filing? is true on the intake" do
    it "returns false, because PR dependents can not get eip3 money" do
      eligibility = described_class.new((create :qualifying_child, intake: create(:ctc_intake, home_location: "puerto_rico", client: create(:client, :with_ctc_return))), MultiTenantService.new(:ctc).current_tax_year)
  ...
  context "when passing in a qualifying child object" do
    ...
    it "uses the passed in object instead of instantiating a new one" do
      described_class.new((create :dependent, intake: create(:ctc_intake)), MultiTenantService.new(:ctc).current_tax_year, child_eligibility: child_eligibility)
  ...
  context "when passing in a qualifying relative object" do
    ...
    it "uses the passed in object instead of instantiating a new one" do
      described_class.new((create :dependent, intake: create(:ctc_intake, client: create(:client, :with_ctc_return))), MultiTenantService.new(:ctc).current_tax_year, relative_eligibility: relative_eligibility)
