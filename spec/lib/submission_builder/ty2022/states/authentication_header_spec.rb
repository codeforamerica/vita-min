...
describe SubmissionBuilder::AuthenticationHeader do
  describe '.build' do
    ...
    context "when a drivers license which does not expire is defined" do
      ...
      let(:intake) { create(:state_file_nc_intake, primary_state_id: state_id) }
      let(:submission) { create(:efile_submission, data_source: intake) }
    ...
    context "when a non North Carolina drivers license is defined" do
    ...
    context "when a North Carolina state issued id is defined" do
      ...
      it "builds an xml with a no additonal info" do
        ...
        expect(doc.at("PrimDrvrLcnsOrStateIssdIdGrp StateIssdIdStCd").text).to eq "NC"
  ...
  describe '#refund_disbursement' do
    ...
    context 'when a submission is receiving a refund with payment_or_deposit_type of mail' do
      before do
        allow_any_instance_of(StateFileNcIntake).to receive(:calculated_refund_or_owed_amount).and_return(100)
        allow_any_instance_of(StateFileNcIntake).to receive(:payment_or_deposit_type).and_return('mail')
    ...
    context 'when a submission is receiving a refund with payment_or_deposit_type of direct deposit' do
      before do
        allow_any_instance_of(StateFileNcIntake).to receive(:calculated_refund_or_owed_amount).and_return(100)
        allow_any_instance_of(StateFileNcIntake).to receive(:payment_or_deposit_type).and_return('direct_deposit')
