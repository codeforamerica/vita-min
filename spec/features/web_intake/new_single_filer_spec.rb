...
RSpec.feature "Web Intake Single Filer", :flow_explorer_screenshot, active_job: true do
  ...
  def intake_after_triage_up_to_documents(intake)
    ...
    page_change_block do
      ...
      expect(page).to have_css("h1", text: I18n.t("views.questions.backtaxes.title"))
    ...
    page_change_block(0.5) do
      ...
      expect(page).to have_css("h1", text: I18n.t("views.questions.start_with_current_year.title", year: current_tax_year))
      click_on "Continue"
    end
    ...
    page_change_block do
      # Phone number
      expect(page).to have_css("h1", text: "Please share your cell phone number.")
    ...
    page_change_block do
      ...
      expect(page).to have_css("h1", text: "Let's verify that contact info with a code!")
    ...
    page_change_block do
      # Email
      expect(page).to have_css("h1", text: "Please share your email address.")
    ...
    page_change_block do
      # Consent form
      expect(page).to have_css("h1", text: I18n.t('views.questions.consent.title'))
    ...
    page_change_block do
      ...
      expect(page).to have_css("h1", text: I18n.t('views.questions.optional_consent.title'))
      toggles = {
    ...
    page_change_block do
      # Chat with us
      expect(page).to have_css("h1", text: "Our team at Virginia Partner is here to help!")
      click_on "Continue"
    end

    page_change_block do
      # Primary filer personal information
      expect(page).to have_css("h1", text: "Select any situations that were true for you in #{current_tax_year}")
    ...
    page_change_block do
      expect(page).to have_css("h1", text: "Have you ever been issued an IP PIN because of identity theft?")
    ...
    page_change_block do
      # Marital status
      expect(page).to have_css("h1", text: "Have you ever been legally married?")
      click_on "No"
    end
    ...
    page_change_block do
      ...
      expect(page).to have_css("h1", text: "Would you like to claim anyone for #{current_tax_year}?")
      click_on "No"
    end

    page_change_block do
      # Students
      expect(page).to have_css("h1", text: I18n.t("views.questions.student.title", year: current_tax_year))
      click_on "Yes"
    end

    page_change_block do
      expect(page).to have_css("h1", text: "In #{current_tax_year}, did you pay any student loan interest?")
      click_on "No"
    end
    ...
    page_change_block do
      expect(page).to have_css("h1", text: "Tell us about your work in #{current_tax_year}")
      click_on "Continue"
    end

    page_change_block do
      # Income from benefits
      expect(page).to have_css("h1", text: "In #{current_tax_year}, did you receive any disability benefits?")
      click_on "No"
    end

    page_change_block do
      # Investment income/loss
      expect(page).to have_css("h1", text: "In #{current_tax_year}, did you have any income from interest or dividends?")
      click_on "No"
    end

    page_change_block do
      expect(page).to have_css("h1", text: "In #{current_tax_year}, did you sell any stocks, bonds, or real estate?")
      click_on "Yes"
    end

    page_change_block do
      expect(page).to have_css("h1", text: "In #{current_tax_year}, did you have any income from the sale of stocks, bonds, or real estate?")
      click_on "No"
    end

    page_change_block do
      expect(page).to have_css("h1", text: "Did you report a loss from the sale of stocks, bonds, or real estate on your #{current_tax_year - 1} return?")
      click_on "Yes"
    end
    ...
    page_change_block do
      ...
      expect(page).to have_css("h1", text: "In #{current_tax_year}, did you have Social Security income, retirement income, or retirement contributions?")
      click_on "No"
    end

    page_change_block do
      # Other income
      expect(page).to have_css("h1", text: "In #{current_tax_year}, did you receive any other money?")
      click_on "Yes"
    end
    ...
    page_change_block do
      # Health insurance
      expect(page).to have_css("h1", text: "Tell us about your health insurance in #{current_tax_year}.")
    ...
    page_change_block do
      # Itemizing
      expect(page).to have_css("h1", text: "Would you like to itemize your deductions for #{current_tax_year}?")
      click_on "No"
    end

    page_change_block do
      expect(page).to have_css("h1", text: "In #{current_tax_year}, did you pay any state, local, real estate, sales, or other taxes?")
      click_on "Yes"
    end

    page_change_block do
      expect(page).to have_css("h1", text: "In #{current_tax_year}, did you receive a state or local income tax refund?")
      click_on "Yes"
    end

    page_change_block do
      # Related to home ownership
      expect(page).to have_css("h1", text: "Have you ever owned a home?")
      click_on "Yes"
    end

    page_change_block do
      expect(page).to have_css("h1", text: "In #{current_tax_year}, did you sell a home?")
      click_on "No"
    end

    page_change_block do
      expect(page).to have_css("h1", text: "In #{current_tax_year}, did you pay any mortgage interest?")
      click_on "No"
    end
    ...
    page_change_block do
      ...
      expect(page).to have_css("h1", text: "In #{current_tax_year}, did you have a loss related to a declared Federal Disaster Area?")
      click_on "No"
    end

    page_change_block do
      expect(page).to have_css("h1", text: "In #{current_tax_year}, did you have debt cancelled or forgiven by a lender?")
      click_on "No"
    end

    page_change_block do
      expect(page).to have_css("h1", text: "In #{current_tax_year}, did you receive any letter or bill from the IRS?")
      click_on "Yes"
    end

    page_change_block do
      expect(page).to have_css("h1", text: "Have you had the Earned Income Credit, Child Tax Credit, American Opportunity Credit, or Head of Household filing status disallowed in a prior year?")
      click_on "Yes"
    end

    page_change_block do
      expect(page).to have_css("h1", text: "In #{current_tax_year}, did you make any estimated tax payments or apply your #{current_tax_year - 1} refund to your #{current_tax_year} taxes?")
      click_on "Yes"
    end

    page_change_block do
      expect(page).to have_css("h1", text: "Did you report a business loss on your #{current_tax_year - 1} tax return?")
      click_on "No"
    end

    page_change_block do
      expect(page).to have_css("h1", text: "Did you purchase a new vehicle in #{current_tax_year}?")
      click_on "Yes"
    end

    page_change_block do
      expect(page).to have_css("h1", text: "In #{current_tax_year}, did you purchase energy efficient home items?")
      click_on "Yes"
    end
  ...
  scenario "new client filing single without dependents fills out document flow" do
    ...
    # IRS guidance
    expect(page).to have_css("h1", text: "First, we need to confirm your basic information.")
    click_on "Continue"

    expect(page).to have_css("h1", text: "Attach a photo of your ID card")
    ...
    expect(page).to have_css("h1", text: I18n.t('views.documents.ssn_itins.title'))
    upload_file("document_type_upload_form_upload", Rails.root.join("spec", "fixtures", "files", "picture_id.jpg"))
    click_on "Continue"

    # Documents: Intro
    expect(page).to have_css("h1", text: I18n.t('views.documents.intro.title'))
    click_on "Continue"

    expect(page).to have_css("h1", text: "Share your employment documents")
    ...
    expect(page).to have_css("h1", text: "Please share any additional documents.")
    upload_file("document_type_upload_form_upload", Rails.root.join("spec", "fixtures", "files", "test-pattern.png"))
    expect(page).to have_content("test-pattern.png")
    click_on "Continue"

    expect(intake.reload.current_step).to end_with("/documents/overview")
    expect(page).to have_css("h1", text: "Great work! Here's a list of what we've collected.")
    ...
    expect(page).to have_css("h1", text: "Success! Your tax information has been submitted.")
    ...
    expect(page).to have_css("h1", text: "Are you willing to answer some additional questions to help us better serve you?")
    click_on "Continue"
    ...
    expect(page).to have_css("h1", text: "What is your race and/or ethnicity?")
    ...
    expect(page).to have_css("h1", text: "Free tax filing")
    ...
    expect(intake.reload.current_step).to end_with("/questions/demographic-primary-race")
    expect(page).to have_css("h1", text: I18n.t("portal.client_logins.new.title"))
  end
