...
RSpec.describe "Filtering clients for bulk actions", active_job: true do
  ...
  let!(:unselected_client) { create :client_with_intake_and_return, tax_return_state: "intake_reviewing", vita_partner: unselected_org }
  ...
  scenario "take action on all filtered clients", js: true do
    ...
    expect(page).to have_text("Successful Bulk Client Organization Update", wait: 5)
    expect(page).to have_text("You successfully moved 30 clients to Rooster Brew.", wait: 5)
  end
end
