...
RSpec.feature "Team member role" do
  context "A logged in team member" do
    ...
    let!(:jerry_visible) {
      create :client,
             vita_partner: vita_partner,
             intake: (build :intake, :filled_out, preferred_name: "Jerry Jujube", primary_consented_to_service: "yes", state_of_residence: "CA"),
             tax_returns: [(build :gyr_tax_return, :intake_in_progress)]
    }
    let!(:abigail_invisible) {
      create :client,
             vita_partner: create(:organization),
             intake: (build :intake, preferred_name: "Abigail Apricot", primary_consented_to_service: "yes", state_of_residence: "CA"),
             tax_returns: [(build :tax_return, :intake_in_progress, year: 2018)]
    }
    let!(:mirabel_invisible) {
      create :client,
             vita_partner: create(:organization),
             intake: (build :intake, preferred_name: "Mirabel Mushroom", primary_consented_to_service: "yes", state_of_residence: "CA"),
             tax_returns: [(build :tax_return, :intake_in_progress, year: 2019)]
    }
