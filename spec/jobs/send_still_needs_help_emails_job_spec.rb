...
RSpec.describe SendStillNeedsHelpEmailsJob, type: :job do
  describe '#perform' do
    ...
    context 'when there are clients with UU as their vita partner' do
      ...
      let!(:uu_site_client) { create(:client, intake: create(:intake), vita_partner: create(:site, parent_organization: urban_upbound)) }
      ...
      context 'and they have tax returns in Ready to File and Not Ready' do
        ...
        let!(:another_ready_tr) { create(:tax_return, client: uu_site_client, status: "intake_ready") }
        let!(:not_ready_tr) { create(:tax_return, client: uu_site_client, status: "intake_in_progress", year: 2018) }

        it 'updates their triggered_still_needs_help_at and tax return statuses' do
          described_class.perform_now

          expect(Client.where.not(triggered_still_needs_help_at: nil)).to include(uu_client, uu_site_client)
        ...
        context 'and they have some tax returns in other statuses' do
          let!(:info_requested_tr) { create(:tax_return, client: uu_site_client, status: "review_info_requested", year: 2017) }

          it 'does not include them and their tax returns in the updates' do
            described_class.perform_now

            expect(Client.where(triggered_still_needs_help_at: nil)).to include(uu_site_client)
        ...
        context 'and they have tax returns in Not Filing' do
          let!(:not_filing_tr) { create(:tax_return, client: uu_site_client, status: "file_not_filing", year: 2017) }

          it 'does include them and their tax returns in the updates' do
            described_class.perform_now

            expect(Client.where.not(triggered_still_needs_help_at: nil)).to include uu_site_client
