  ...
  describe '#response_needed_breaches' do
    ...
    context "processing on a Friday @ 10:05am UTC" do
      before do
        ...
        # not in breach
        client4 = create(:client, vita_partner_id: vita_partner_2.id,  tax_returns: [create(:tax_return, :prep_ready_for_prep)]) # no breach
        Timecop.freeze(t.prev_occurring(:wednesday)) { client4.flag! }

        client5 = create(:client, vita_partner_id: vita_partner_2.id, tax_returns: [create(:tax_return, :prep_ready_for_prep)]) # no breach
        client5.clear_flag!
      end
    ...
    context "processing on a Monday" do
      before do
        ...
        # not in breach
        create(:client, flagged_at: nil, vita_partner_id: vita_partner_2.id, tax_returns: [create(:tax_return, :prep_ready_for_prep)]) # no breach
      end
