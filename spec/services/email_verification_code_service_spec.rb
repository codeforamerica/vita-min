  ...
  describe ".request_code" do
    ...
    context "when service_type is ctc" do
      ...
      it "creates a VerificationEmail, sends an email, and creates an EmailAccessToken object" do
        ...
        expect(email.body.encoded).to include "Your six-digit code GetCTC verification code is: 123456"
    ...
    context "when service type is GYR" do
      ...
      it "sends an email that includes 'GetYourRefund'" do
        ...
        email = ActionMailer::Base.deliveries.last
        expect(email.to).to eq [email_address]
        expect(email.body.encoded).to include "Your six-digit code GetYourRefund verification code is: 123456"
      end
    end
    ...
    context "when service type is statefile" do
      ...
      it "sends an email that includes 'FileYourStateTaxes'" do
        ...
        email = ActionMailer::Base.deliveries.last
        expect(email.to).to eq [email_address]
        expect(email.body.encoded).to include "Your six-digit code FileYourStateTaxes verification code is: 123456"
      end
    end
  end
end