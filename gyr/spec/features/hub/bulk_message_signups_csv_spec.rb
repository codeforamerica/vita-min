...
RSpec.describe "Uploading a CSV for bulk messaging of signups", active_job: true do
  ...
  let!(:ctc_email_and_phone_signup) { create :ctc_signup, email_address: "ctcfriend@example.com", phone_number: "+15855551212" }
  ...
  context "for GetCTC signups" do
    around do |example|
      @filename = Rails.root.join("tmp", "bulk-signups-message-getctc-test-#{SecureRandom.hex}.csv")
      File.write(@filename, <<~CSV)
        id
        #{ctc_email_and_phone_signup.id}
      CSV
      example.run
      File.unlink(@filename)
    end

    scenario "bulk messaging signups by signup CSV" do
      visit hub_signup_selections_path
      ...
      expect(OutgoingMessageStatus.count).to eq 1
      expect(OutgoingMessageStatus.last.parent.email_address).to eq(ctc_email_and_phone_signup.email_address)
      expect(BulkSignupMessage.last.subject).to eq 'my great email subject'
      ...
      expect(OutgoingMessageStatus.count).to eq 2
      expect(OutgoingMessageStatus.last.parent.phone_number).to eq(ctc_email_and_phone_signup.phone_number)
    end
  end
end
