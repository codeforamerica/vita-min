...
RSpec.describe Hub::BulkActionForm do
  ...
  describe "#valid?" do
    describe "#no_missing_message_locales" do
      context "when clients with both locales exist but one is missing" do
        let!(:client_en) { create :client, intake: build(:intake, locale: "en"), tax_returns: [(build :gyr_tax_return, tax_return_selections: [tax_return_selection])] }
        let!(:client_es) { create :client, intake: build(:intake, locale: "es"), tax_returns: [(build :gyr_tax_return, tax_return_selections: [tax_return_selection])] }
        let(:form_params) do
          {
  ...
  describe "setting default values" do
    context "default message body" do
      let(:intake_en) { build(:intake, locale: "en", preferred_name: "Luna Lemon") }
      let(:intake_es) { build(:intake, locale: "es", preferred_name: "Robby Radish") }
      let!(:client_en) { create :client, intake: intake_en, tax_returns: [(build :gyr_tax_return, tax_return_selections: [tax_return_selection])] }
      let!(:client_es) { create :client, intake: intake_es, tax_returns: [(build :gyr_tax_return, tax_return_selections: [tax_return_selection])] }
