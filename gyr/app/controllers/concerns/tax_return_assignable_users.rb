  ...
  def assignable_users(client, included_users = [])
    ...
    if client.vita_partner.present?
      if client.vita_partner.site?
        ...
        site_coordinators = User.where(role: SiteCoordinatorRole.assignable_to_sites([client.vita_partner]))
        ...
        org_team_members = User.where(role: TeamMemberRole.assignable_to_sites(child_sites))
        org_site_coordinators = User.where(role: SiteCoordinatorRole.assignable_to_sites(child_sites))
