...
#         record ||= current_client&.intake || Intake.find_by_id(session[:intake_id])
#         @eager_loaded_current_intake = if record.present?
#                                          record.class.includes(state_file_1099rs: :state_specific_followup).find(record.id)
#                                        end
#       end
