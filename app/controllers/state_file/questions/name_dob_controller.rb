module StateFile
  module Questions
    class NameDobController < AuthenticatedQuestionsController
      ...
      def form_params
        permitted_attributes = [:primary_first_name, :primary_middle_initial, :primary_last_name, :device_id]
        permitted_attributes.concat([:primary_birth_date_month, :primary_birth_date_day, :primary_birth_date_year])
        permitted_attributes.concat([:spouse_first_name, :spouse_middle_initial, :spouse_last_name]) if current_intake.ask_spouse_name?
