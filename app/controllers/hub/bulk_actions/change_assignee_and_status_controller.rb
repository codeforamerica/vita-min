  ...
  module BulkActions
    class ChangeAssigneeAndStatusController < BaseBulkActionsController
      ...
      def load_current_tax_return_statuses
        @current_tr_statuses = @tax_return_selection.tax_returns.includes(:client, :assigned_user).joins(:tax_return_transitions).where(tax_return_transitions: { most_recent: true }).map(&:current_state).uniq
      end
