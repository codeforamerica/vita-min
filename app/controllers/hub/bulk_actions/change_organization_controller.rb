  ...
  module BulkActions
    class ChangeOrganizationController < BaseBulkActionsController
      ...
      def load_current_vita_partner_names
        @current_vita_partner_names = VitaPartner.where(clients: @tax_return_selection.clients).pluck(:name).uniq.sort
      end

      def create_change_org_notifications!
        if @new_vita_partner.present?
          bulk_update = BulkClientOrganizationUpdate.create!(tax_return_selection: @tax_return_selection, vita_partner: @new_vita_partner)
