module Hub
  class UserNotificationsController < Hub::BaseController
    ...
    def index
      ...
      @user_notifications = current_user.notifications.includes(:notifiable).where('created_at >= ?', Date.new(Rails.configuration.product_year)).order(created_at: :desc).page(params[:page])
    end
