class ClientLoginService
  class << self
    ...
    def handle_email_request(email_address:, visitor_id:)
    ...
    def handle_sms_request(phone_number:, visitor_id:)
    ...
    def can_login_by_email_verification?(email_address)
      accessible_intakes.where(email_address: email_address).or(accessible_intakes.where(spouse_email_address: email_address))
    end

    def can_login_by_sms_verification?(sms_phone_number)
      accessible_intakes.where(phone_number: sms_phone_number).or(accessible_intakes.where(sms_phone_number: sms_phone_number))
    end
