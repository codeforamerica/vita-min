class PartnerRoutingService
  ...
  def vita_partner_from_zip_code
    ...
    eligible_with_capacity = @base_orgs.with_capacity.joins(:serviced_zip_codes)
                                 .where(vita_partner_zip_codes: { zip_code: @zip_code }).limit(1)
  ...
  def route_to_national_overflow_partner
    national_overflow_locations = @base_vita_partners.where(national_overflow_location: true).order(Arel.sql('RANDOM()'))
