...
class StateFileW2 < ApplicationRecord
  ...
  validates_numericality_of :state_income_tax_amt, only_integer: true, message: I18n.t('errors.messages.whole_number'), if: -> { state_income_tax_amt.present? }
  validates :state_income_tax_amt, numericality: { greater_than_or_equal_to: 0 }, if: -> { state_income_tax_amt.present? }
  validates_numericality_of :local_wages_and_tips_amt, only_integer: true, message: I18n.t('errors.messages.whole_number'), if: -> { local_wages_and_tips_amt.present? }
