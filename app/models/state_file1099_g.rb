...
class StateFile1099G < ApplicationRecord
  ...
  validates :payer_name, :presence => {:message => I18n.t("errors.attributes.payer_name.blank") }, format: { with: /(([A-Za-z0-9#-()]|&|') ?)*([A-Za-z0-9#-()]|&|')/.freeze, message: ->(_object, _data) { I18n.t("errors.attributes.payer_name.invalid") }}
  validates :payer_street_address, :presence => {:message => I18n.t("errors.attributes.address.street_address.blank") }, format: { with: /\A[a-zA-Z0-9\/\s-]+\z/.freeze, message: ->(_object, _data) { I18n.t("errors.attributes.address.street_address.invalid") }}
  validates :payer_city, :presence => {:message => I18n.t("errors.attributes.address.city.blank")}, format: { with: /\A[a-zA-Z\s]+\z/.freeze, message: ->(_object, _data) { I18n.t("errors.attributes.address.city.invalid") }}
