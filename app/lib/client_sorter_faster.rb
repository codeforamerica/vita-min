class ClientSorterFaster
  ...
  def filtered_and_sorted_clients(default_order: nil)
    ...
    filtered_clients.delegated_order_faster(@sort_column, @sort_order)
  end
  ...
  def filtered_clients
    clients = if current_user&.greeter?
                ...
                @clients.greetable_faster.or(Client.where('ARRAY[?] && filterable_tax_return_assigned_users', current_user.id))
              else
    ...
    if @filters[:stage].present?
      clients = clients.where('ARRAY[?]::varchar[] && filterable_tax_return_states', TaxReturnStateMachine::STATES_BY_STAGE[@filters[:stage]])
    end
    if limited_user_ids.present?
      # TODO: this is attempting to replicate the behavior where filtering to 'unassigned' and/or 'assigned to me' and/or 'assigned to'
      # at the same time is allowed and should do an 'or'. That's probably not needed, so we could probably simplify.
      real_ids = limited_user_ids.compact
      has_null = limited_user_ids.include?(nil)
      if real_ids.present? && has_null
        clients = clients.where('ARRAY[?] && filterable_tax_return_assigned_users', real_ids).or(clients.where('array_position(filterable_tax_return_assigned_users, NULL) IS NOT NULL'))
      elsif real_ids.present?
        clients = clients.where('ARRAY[?] && filterable_tax_return_assigned_users', real_ids)
      elsif has_null
        clients = clients.where('array_position(filterable_tax_return_assigned_users, NULL) IS NOT NULL')
      end
    end
    if @filters[:year].present?
      clients = clients.where('ARRAY[?] && filterable_tax_return_years', @filters[:year].to_i)
    end
    if @filters[:status].present?
      clients = clients.where('ARRAY[?]::varchar[] && filterable_tax_return_states', @filters[:status])
    end
    if @filters[:active_returns].present?
      clients = clients.where.not('filterable_tax_return_states <@ ARRAY[?]::varchar[]', TaxReturnStateMachine::EXCLUDED_FROM_SLA)
    end
    if @filters[:service_type].present?
      clients = clients.where('ARRAY[?]::varchar[] && filterable_tax_return_service_types', @filters[:service_type])
    end
