          ...
          class Id39R < SubmissionBuilder::Document
            ...
            def document
              build_xml_doc("Form39R") do |xml|
                ...
                add_element_if_present(xml, "PensionFilingStatusAmount", :ID39R_B_LINE_8a) if Flipper.enabled?(:show_retirement_ui)
                add_element_if_present(xml, "SocialSecurityBenefits", :ID39R_B_LINE_8c) if Flipper.enabled?(:show_retirement_ui)
                add_element_if_present(xml, "PensionExclusions", :ID39R_B_LINE_8e) if Flipper.enabled?(:show_retirement_ui)
