module Ctc
  class DriversLicenseForm < QuestionsForm
    ...
    def save
      @intake.update(drivers_license_attributes: attributes_for(:drivers_license).merge(
    ...
    def self.existing_attributes(intake, _attribute_keys)
      if intake.drivers_license.present?
        drivers_license = intake.drivers_license
        super.merge(
    ...
    def issue_date
      parse_date_params(issue_date_day, issue_date_month, issue_date_year)
    end

    def expiration_date
      parse_date_params(expiration_date_day, expiration_date_month, expiration_date_year)
    end

    def issue_date_is_valid_date
      valid_text_date(issue_date_day, issue_date_month, issue_date_year, :issue_date)
    end

    def expiration_date_is_valid_date
      valid_text_date(expiration_date_day, expiration_date_month, expiration_date_year, :expiration_date)
    end
  end
end
